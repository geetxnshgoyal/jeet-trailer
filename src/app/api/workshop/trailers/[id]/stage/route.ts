 ...
 *
 * `{ action: "start", stageIndex? }` — the worker picks up that stage (claiming
 * it if unassigned). `{ action: "complete", stageIndex?, notes? }` — marks it
 * done; the trailer finishes once every stage is complete. Stages are
 * independent, so any pending one can be started at any time. Omitting
